...
function Synthesis({ note }: { note: Note }) {
  const [output, setOutput] = useState('');

  const clickedMe = useCallback(() => {
    if (note.text) {
      //call api
      //set output
      let uri =
        'https://api.mantiumai.com/v1/prompt/5e81f081-1fc8-47dc-9de2-2d5dfec4a82b/try?input=' +
...
*/
  ...
  return (
    <Popover>
      <PopoverTrigger>
        <Button colorScheme='green' onClick={clickedMe}>
          Synthesize!
        </Button>
      </PopoverTrigger>
      <PopoverContent>
  ...
  //     <br></br>
  //     <br></br>
  //     <Button colorScheme='purple' onClick={clickedMe}>
  //       {' '}
  //       Submit
  //     </Button>
  ...
  //     <Center bg='white' h='12px' color='black'>
  //       <div>{output}</div>
  //     </Center>
  //   </div>
  // );
